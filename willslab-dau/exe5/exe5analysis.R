print("DAU: EXE5")
# Andy Wills - 24 Oct 2014

print("Classic analysis")
# Load data
cdta <- read.csv("exe5classic.csv")
...
# Note: The UD objective sort analysis is very close to significance (p = .054)
# whereas the combined sort was in the same direction but not significant (p = .17)
# This difference between the two analysis methods does not change the conclusions
# of the study.


t.test(kd[,2]~kd[,1], var.equal = TRUE) #UD
t.test(kd[,3]~kd[,1], var.equal = TRUE) #OS
t.test(kd[,4]~kd[,1], var.equal = TRUE) #OT
