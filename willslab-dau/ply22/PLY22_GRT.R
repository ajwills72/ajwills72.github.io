...
print("Remove participants with accuracy <= 0.4")
resultsGRT <- resultsGRT[resultsGRT$Accuracy>0.4,]
...
resultsGRT [c('Model','minBIC','wBIC','BIC.RND','BIC.RND_BIAS',
              'BIC.UDF', 'BIC.UDO','BIC.CJ_UR','BIC.CJ_UL',
              'BIC.CJ_LL','BIC.CJ_LR', 'BIC.GLC','w.RND','w.RND_BIAS',
              'w.UDF','w.UDO','w.CJ_UR', 'w.CJ_UL','w.CJ_LL',
              'w.CJ_LR','w.GLC')] <- NA

for(i in 1:nrow(resultsGRT)){
    print(i)
    data_onesub <- subset(trialData, Participant==resultsGRT$Participant[i] & 
...
counts <- table(resultsGRT$Condition, resultsGRT$Model, 
                resultsGRT$Block)[,c(2,1,5,3,4),]

results <- counts/c(17,20)
...
print("Appendix Table  (Experiment 1):")
