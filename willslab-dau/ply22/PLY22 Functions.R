...
grtmodel <- function(onesub,ic="BIC"){
    ...
    ## Fit UD (length)
    fit.1dfreq <- glc(Response ~ Frequency, data=onesub,  zlimit=7)

    ## Fit UD (angle)
    fit.1dori <- glc(Response ~ Orientation, data=onesub,  zlimit=7) 

    ## Fit CJ1 (upper right)
    fit.cjur <- gcjc(Response ~ Frequency + Orientation, data=onesub,
                     config=1, zlimit=7, equal.noise=TRUE) 

    ## Fit CJ2 (upper left)
    fit.cjul <- gcjc(Response ~ Frequency + Orientation, data=onesub,
                     config=2, zlimit=7, equal.noise=TRUE) 

    ## Fit CJ3 (lower left)
    fit.cjll <- gcjc(Response ~ Frequency + Orientation, data=onesub,
                     config=3, zlimit=7, equal.noise=TRUE) 

    ## Fit CJ4 (lower right) 
    fit.cjlr <- gcjc(Response ~ Frequency + Orientation, data=onesub,
                     config=4, zlimit=7, equal.noise=TRUE)

    ## Fit GLC
    fit.glc <- glc(Response ~ Frequency + Orientation, data=onesub,
                   zlimit=7, covstruct="diagonal")
    ...
    if(ic=="BIC"){
        ...
        bics <- c(bic.rnd,bic.rndBias,bic.1dfreq,bic.1dori,bic.cjur,
                  bic.cjul, bic.cjll,bic.cjlr,bic.glc)
    }
